...
from_list_test() ->
    gleam@should:equal(gleam@map:size(gleam@map:from_list([{4, 0}, {1, 0}])), 2),
    gleam@should:equal(
...
update_test() ->
    ...
    gleam@should:equal(
        gleam@map:update(Dict, <<"a"/utf8>>, Inc_or_zero),
        gleam@map:from_list(
            [{<<"a"/utf8>>, 1}, {<<"b"/utf8>>, 1}, {<<"c"/utf8>>, 2}]
        )
    ),
    gleam@should:equal(
        gleam@map:update(Dict, <<"b"/utf8>>, Inc_or_zero),
        gleam@map:from_list(
            [{<<"a"/utf8>>, 0}, {<<"b"/utf8>>, 2}, {<<"c"/utf8>>, 2}]
        )
    ),
    gleam@should:equal(
        gleam@map:update(Dict, <<"z"/utf8>>, Inc_or_zero),
        gleam@map:from_list(
            [{<<"a"/utf8>>, 0},
             {<<"b"/utf8>>, 1},
             {<<"c"/utf8>>, 2},
...
fold_test() ->
    ...
    Concat = fun(K, _, Acc@1) -> gleam@string:append(Acc@1, K) end,
    gleam@should:equal(
